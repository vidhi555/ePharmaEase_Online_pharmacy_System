                        ...
                        <div class="order-card shadow-sm border-0 rounded-4 p-4 ">
                            ...
                            try {
                                if ($fetch_user) {
                                   ...
                                   $order_detail = $conn->prepare("SELECT *
...
ORDER BY o.o_id DESC");
                                    ...
                                    foreach ($fetch_oredr as $o) {
                                        if($o['order_status'] == 'cancelled'){
                                            $class = 'border-left: 6px solid #dc3545;';
                                            
                                        }
                            ?>
<?php foreach ($fetch_oredr as $order) { ?>
        <div class="col-lg-6 col-md-6">
            <div class="order-card shadow-sm">
                ...
                <?php 
                $inside_loop = $conn->prepare("SELECT * FROM ep_orders_master o
JOIN ep_orders_items oi ON o.o_id = oi.o_id
JOIN ep_products p ON p.p_id = oi.p_id
WHERE oi.o_id = :id
ORDER BY o.o_id DESC");
...
$fetch_inside = $inside_loop->fetchAll(PDO::FETCH_ASSOC);
        ...
        <?php } ?>
                                        ...
                                        <div class="order-item mb-3 p-3 rounded-3 bg-light" style="<?= $class ?>">
                                            <p class="badge-date"><?= date('d-m-Y',strtotime($o['oder_date']))?> </p>
<!-- 
                                            ...
                                            <?php } ?>
                                            <img class="img_order" src="../LearnAdmin/All_images_uploads/<?= $o['image'] ?>" alt="">
                                            ...
                                            <p class="<?= ($o['order_status']=='cancelled')?'badge-cancel':'badge-date' ?>"><?= $o['order_status'] ?></p>
